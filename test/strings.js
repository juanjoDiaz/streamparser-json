...
for (const stringBufferSize of [0, 64 * 1024]) {
  ...
  test('multibyte characters', (t) => {
    ...
    t.test('chunking', (t) => {
      ...
      t.test('2 byte utf8 \'De\' character chunked inbetween 1st and 3nd byte: д', (t) => {
        t.plan(1);
      ...
      t.test('3 byte utf8 \'Han\' character chunked inbetween 2nd and 3rd byte: 我', (t) => {
        t.plan(1);
      ...
      t.test('4 byte utf8 character (unicode scalar U+2070E) chunked inbetween 2nd and 3rd byte: 𠜎', (t) => {
        t.plan(1);
      ...
      t.test('1-4 byte utf8 character string chunked inbetween random bytes: Aж文𠜱B', (t) => {
        t.plan(11);

        const p = new JsonParser({ stringBufferSize });
        p.onValue = (value) => t.equal(value, 'Aж文𠜱B');

        const eclectic_buffer = new Uint8Array([0x41, // A
                                          0xd0, 0xb6, // ж
                                          0xe6, 0x96, 0x87, // 文
